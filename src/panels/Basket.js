...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  ...
  const [ selfService, setSelfService ] = useState(JSON.parse(localStorage.getItem('selfService')) || false);
  ...
  const setService = () => {
    const selfServiceMod = !selfService;
    setSelfService(selfServiceMod);
    localStorage.setItem('selfService', selfServiceMod);
  }
  ...
  return (
    <div className="Place">
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          <h3>С собой</h3>
          <Checkbox checked={!selfService} onToggle={setService} />
        </div>
        <div className="Place__choice-item">
          <h3>На месте</h3>
          <Checkbox checked={selfService} onToggle={setService} />
      ...
      <footer className="Place__footer">
        <Link to={`/order/${area.id}/${item.id}`} className={(products.length > 0) ? "Place__order": "Place__order isDisabled"}>
          Оплатить {price}
        </Link>
      </footer>
    </div>
